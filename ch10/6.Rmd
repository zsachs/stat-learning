...
### b
Since the loading vectors are unit vectors, the first principal component scores
$z_{i1}=X_i\cdot\phi_1$ are the scalar projections of each gene vector on the
first principal component. Thus, the rows of the new matrix are the vector
rejection of each gene vector on the first principal component. That is:
$$
X_i-\frac{X_i\cdot\phi_1}{\phi_1\cdot\phi_1}\phi_1
=X_i-z_{i1}\phi_1
=(x_{ij}-z_{i1}\phi_{j1})_j.
$$
Rejecting onto the first principal component removes the 10% of the variation
in the data correlated with the machine used, hopefully leaving only variation
attributable to genes and group status.
